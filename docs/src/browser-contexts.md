...
* browser contexts
  * == isolated clean-slate environments
    * isolated
      * == OWN
        * local storage
        * session storage
        * cookies
        * ...
      * -> test1 is COMPLETELY isolated -- from -- test2
    * are
      * fast
      * cheap to create
    * -> pros
  ...
  * == incognito-like profiles
  * uses
    * place | execute tests / written -- with -- Playwright
      * == Playwright creates 1 browser context / EACH test
      * provides a default [Page] in that context.

## Why is Test Isolation Important?

- No failure carry-over
* If one test fails it doesn't affect the other test.
...
* The problem with cleaning up in between tests is that it can be easy to forget to clean up and some things are impossible to clean up such as "visited links"
* State from one test can leak into the next test which could cause your test to fail and make debugging harder as the problem comes from another test
...
* Check out our [Emulation](Playwright Test/emulation.md) guide for more details.

## Multiple Contexts in a Single Test

Playwright can create multiple browser contexts within a single scenario
* This is useful when you want to test for multi-user functionality, like a chat.
